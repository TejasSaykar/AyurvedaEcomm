...
exports.createCategory = async (req, res) => {
    ...
    try {
        ...
        const existCat = await categoryModel.findOne({ name: category });
        if (existCat) {
            return res.json({ message: "Category already present" });
        }

        const newCategory = await new categoryModel({ name: category }).save();
...
exports.singleCategory = async (req, res) => {
    ...
    try {
        const category = await categoryModel.findByIdAndUpdate({ _id: id }, { $set: req.body }, { new: true });
        return res.status(200).json({
            success: true,
